...
public class LocalStack extends Stack {
    ...
    public LocalStack(final App scope, final String id, final StackProps props) {
        ...
        FargateService authService = createFargateService(
                ...
                List.of(4005),
                authServiceDb,
                Map.of("JWT_SECRET", "xa31Hqfx4IjdZq7TtmGy60whzakFIdaFmalqOXo9AuJCcbVhLjdJzDO2CMP0")
        );
    ...
    public static void main(final String[] args) {
        ...
        System.out.println("App synthesizing in progress..."); // ← runs before
        app.synth();
        System.out.println("App synthesized successfully!");
    }
    ...
    private DatabaseInstance createDatabase(String id, String dbName) {
        return DatabaseInstance.Builder.create(this, id).engine(DatabaseInstanceEngine.postgres(PostgresInstanceEngineProps.builder().version(PostgresEngineVersion.VER_17_2).build())).vpc(vpc).instanceType(InstanceType.of(InstanceClass.BURSTABLE2, InstanceSize.MICRO)).allocatedStorage(20).credentials(Credentials.fromGeneratedSecret("admin_user")).databaseName(dbName).removalPolicy(RemovalPolicy.DESTROY) // remove data hen remove the stack
                .build();
    }

    private CfnHealthCheck createDbHealthCheck(DatabaseInstance db, String id) {
        return CfnHealthCheck.Builder.create(this, id).healthCheckConfig(CfnHealthCheck.HealthCheckConfigProperty.builder().type("TCP").port(db.getInstanceEndpoint().getPort()).ipAddress(db.getDbInstanceEndpointAddress()).requestInterval(30).failureThreshold(3).build()).build();
    }

    private CfnCluster createMskCluster() { // creating managed kafka cluster using msk
        return CfnCluster.Builder.create(this, "MskCluster").clusterName("kafka-cluster").kafkaVersion("2.8.0").numberOfBrokerNodes(1) // no of brokers
                .brokerNodeGroupInfo(CfnCluster.BrokerNodeGroupInfoProperty.builder().instanceType("kafka.m5.xlarge").clientSubnets(vpc.getPrivateSubnets().stream().map(ISubnet::getSubnetId).collect(Collectors.toList())).brokerAzDistribution("DEFAULT").build()).build();
    }
    ...
    private FargateService createFargateService(String id, String imageName, List<Integer> ports, DatabaseInstance db, Map<String, String> additionalEnvVars) {
        ...
        if(db != null){
            envVars.put("SPRING_DATASOURCE_URL","jdbc:postgressql://%s:%s/%s-db".formatted(
    ...
    private void createApiGatewayService(){
        ...
        ContainerDefinitionOptions containerOptions = ContainerDefinitionOptions.builder()
                ...
                .environment(Map.of(
                        ...
                        "AUTH_SERVICE_URL", "http://host.docker.interval:4005"
                ))
